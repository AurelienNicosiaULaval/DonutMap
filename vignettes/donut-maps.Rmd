...
3. `donut_leaflet()` creates an interactive `leaflet` map with clickable
   donut segments and optional flow lines.
...
category values and flows are simulated for demonstration.
...
`donut_leaflet()` returns a `leaflet` htmlwidget. Donut segments and flow lines
can be clicked to open popups, and hover labels are enabled by default.
